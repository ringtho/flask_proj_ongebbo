...
def add_redflag():
    ...
    try:
        print(data)
        if type(data["type"]) is not string:
            raise ValueError("Red Flag type should be a string")
        redflag = RedFlag(data["createdBy"], data["type"], data["location"],
        data["status"], data["images"], data["videos"], data["comment"])
        redflags.append(redflag)
    except ValueError as e:
        print(e)
        return jsonify({"status": 400, "message": "Red Flag type should be a string"}), 400
    return jsonify({"status": 201, "data": [redflags.json_format()]}), 201
